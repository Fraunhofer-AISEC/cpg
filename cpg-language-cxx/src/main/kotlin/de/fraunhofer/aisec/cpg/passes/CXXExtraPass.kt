...
class CXXExtraPass(ctx: TranslationContext) : ComponentPass(ctx) {
    ...
    /**
     * This function connects a [Func] that is a definition (i.e., has a body) to possible
     * declarations of the same function (has [Func.isDefinition] set to false) pointing to it by
     * setting the field [Func.definition].
     *
    ...
    private fun connectDefinitions(declaration: Node) {
        ...
        val candidates =
            scope.symbols[declaration.symbol]?.filterIsInstance<Func>()?.filter {
