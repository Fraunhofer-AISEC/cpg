...
class CXXTemplateTest {
    ...
    fun testConstructor() {
        ...
        var method =
            tu.dMethods.firstOrNull { it.signature == "MyTemplateClass(T&&)MyTemplateClass" }
        ...
        method = tu.dMethods.firstOrNull { it.signature == "MyClass(MyClass&&)MyClass" }
        assertIs<ConstructorDeclaration>(method)
        assertNotNull(method)
    }
    ...
    fun testFunctionTemplateCall() {
        ...
        assertNotNull(tu)
        assertTrue(tu.dFunctions.all { !it.isInferred })
    }
}
