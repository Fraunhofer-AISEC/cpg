...
class CPPLambdaTest {
    ...
    fun testLambdaForeach() {
        ...
        val lambda = function.calls["for_each"]?.arguments?.get(2) as? LambdaExpr
        assertNotNull(lambda)
    ...
    fun testLambdaInitializedCaptures() {
        ...
        val lambdaVar = function.variables["this_is_a_lambda"]
        assertNotNull(lambdaVar)
        val lambda = (lambdaVar.initializer as? CallExpr)?.callee as? LambdaExpr
        assertNotNull(lambda)
    }
    ...
    fun testLambdaFunctionNull() {
        ...
        val lambdaVar = function.variables["this_is_a_lambda"]
        assertNotNull(lambdaVar)
        assertNotNull(lambdaVar.initializer as? LambdaExpr)
        assertNotNull((lambdaVar.initializer as? LambdaExpr)?.function)
    }
}
