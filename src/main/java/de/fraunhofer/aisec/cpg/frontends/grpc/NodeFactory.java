...
public class NodeFactory {
  ...
  public static Node createNode(
      ...
      List<Node> cpgNodes) throws TranslationException {
    // Sanity check
    ...
    /*    
    There are 2 different Functions: leaves and no-leaves
    Leaves will create the Node itself and will add it to the list.
    No-leaves will skip that part

    Leaves:
    1. Create Node itself
    2. Add node to list (only in leave functions)
  ...
  private static Declaration createDeclaration(
    ...
    if (d.hasTranslationUnitDeclaration()) {
      declaration = createTranslationUnitDeclaration(d.getTranslationUnitDeclaration(), index, grpcNodes, cpgNodes);
      ...
      throw new RuntimeException("Error in NodeFactory!");
    }
  ...
  private static ValueDeclaration createValueDeclaration(
    ...
    if (v.hasVariableDeclaration()) {
      valueDeclaration = createVariableDeclaration(v.getVariableDeclaration(), index, grpcNodes, cpgNodes);
    } else {
      throw new RuntimeException("Error in NodeFactory!");
    }
