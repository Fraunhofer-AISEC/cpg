...
class RunPassTest {
    ...
    @Test
    fun serverAddsToolsAndRunsPassOnSelectedNode() = withMcpServer { server, client ->
    ...
    @Test
    fun serverListsPassesWithRequiredNodeType() = withMcpServer { _, client ->
        val result =
            client.callTool(
                CallToolRequest(
    ...
    @Test
    fun serverTranslateToolProducesNodes() = withMcpServer { _, client ->
        val result =
            client.callTool(
                CallToolRequest(
                    CallToolRequestParams(
        ...
        assertTrue(
            parsed.functionSummaries.any { it.name.contains("hello") || it.name.contains("print") }
        )
    }

    @Test
    fun testNoCtx() = withMcpServer { _, client ->
        translate(client)
    ...
    @Test
    fun testInvalidNodeId() = withMcpServer { _, client ->
        translate(client)
    ...
    @Test
    fun testInvalidPassName() = withMcpServer { _, client ->
        translate(client)
                    ...
                    CallToolRequestParams(
                        ...
                        arguments =
                            buildJsonObject {
                                ...
                                put("nodeId", globalAnalysisResult.calls["print"]!!.id.toString())
                            },
                    )
                )
            )
    ...
    @Test
    fun testExecuteSamePassTwice() = withMcpServer { _, client ->
        translate(client)
