...
class StatementTest {
    ...
    fun testBranchStatement() {
        ...
        val start = main.allDescendants<LabelStatement>().firstOrNull { it.label == "start" }
        assertNotNull(start)

        val cases = start.allDescendants<CaseStatement>()
        ...
        val default = start.allDescendants<DefaultStatement>().firstOrNull()
        assertNotNull(default)

        val end = main.allDescendants<LabelStatement>().firstOrNull { it.label == "end" }
        assertNotNull(end)
    }
    ...
    fun testDeferStatement() {
        ...
        val `do` = p.allMethods["Do"]
        assertNotNull(`do`)

        val op = `do`.allDescendants<UnaryOperator> { it.name.localName == "defer" }.firstOrNull()
        assertNotNull(op)
