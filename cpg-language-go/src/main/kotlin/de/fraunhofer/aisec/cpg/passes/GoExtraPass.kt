 ...
 *
 * This pass also adjusts the names of keys in a [ast.statements.expressions.KeyValueExpression], which is part of an
 * [ast.statements.expressions.InitializerListExpression] to a fully-qualified name that contains the name of the [ObjectType]
 ...
 *
 * This pass also adds methods of [ast.declarations.RecordDeclaration.embeddedStructs] into the scope of the
 * [ast.declarations.RecordDeclaration] itself, so that it can be resolved using the regular [SymbolResolver].
 */
...
class GoExtraPass(ctx: TranslationContext) : ComponentPass(ctx) {
    ...
    /**
     * This function adds methods of [ast.declarations.RecordDeclaration.embeddedStructs] into the scope of the
    ...
    /**
     * handleInitializerListExpression changes the references of keys in a [ast.statements.expressions.KeyValueExpression] to
    ...
    /**
     * This function gets called for every [ast.statements.expressions.AssignExpression], to check, whether we need to
    ...
    private fun handleAssign(assign: AssignExpression) {
        ...
        for ((idx, expr) in assign.lhs.withIndex()) {
            if (expr is Reference) {
                ...
                val ref = scopeManager.lookupSymbolByNodeNameOfType<de.fraunhofer.aisec.cpg.graph.ast.declarations.VariableDeclaration>(expr)
                if (ref.isEmpty()) {
