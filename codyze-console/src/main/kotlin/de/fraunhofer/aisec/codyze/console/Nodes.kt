...
fun Node.toJSON(noEdges: Boolean = false): NodeJSON {
    return NodeJSON(
        ...
        translationUnitId = this.pTranslationUnit?.id,
        componentName = this.pComponent?.name?.toString(),
    )
}
...
fun Result.toJSON(): FindingsJSON {
    ...
    return FindingsJSON(
        kind = this.kind?.name ?: "",
        path = path.toString(),
        component = translationUnit?.pComponent?.name?.toString(),
        rule = this.ruleID,
