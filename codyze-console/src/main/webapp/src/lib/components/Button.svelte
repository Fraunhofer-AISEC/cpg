<script lang="ts">
  ...
  const variantClasses = {
    ...
    ghost: 'text-gray-500 hover:text-gray-700 hover:bg-gray-100 focus:ring-gray-500 disabled:text-gray-300'
  };
...
{:else}
  <button 
    {type}
    disabled={isDisabled}
    class={`${combinedClasses} ${isDisabled ? 'opacity-70 cursor-not-allowed' : ''}`}
    {onclick}
    {...restProps}
  >
    {#if loading}
