...
import type { AnalysisResultJSON, McpCapabilities, SkillInfo } from '$lib/types';
...
export const load: PageLoad = async ({ fetch }) => {
  try {
    ...
    const [resultRes, capsRes, skillsRes] = await Promise.all([
      fetch('/api/result'),
    ...
    return { result, mcpCapabilities, skills };
  } catch (error) {
    if (error && typeof error === 'object' && 'status' in error) throw error;
    console.error('Error loading chat page:', error);
    return { result: null, mcpCapabilities: null, skills: [] };
  }
};
