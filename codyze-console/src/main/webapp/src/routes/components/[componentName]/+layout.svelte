...
<div class="h-full">
  ...
  <div class="flex h-[calc(100vh-180px)] overflow-hidden rounded-lg border border-gray-200 bg-white">
    ...
    <div class="w-72 overflow-auto border-r border-gray-200">
      <nav class="p-4">
        <h2 class="mb-3 text-xs font-semibold uppercase text-gray-500">Files</h2>
              ...
              <li>
                {#if node.type === 'component'}
                  ...
                  <a
                    ...
                    class="flex w-full items-center rounded-md px-2 py-1 text-left text-sm font-semibold {node.isCurrent ? 'bg-indigo-100 text-indigo-800 border border-indigo-200' : 'text-indigo-700 hover:bg-indigo-50'}"
                    style="padding-left: {depth * 12 + 8}px"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="mr-2 h-4 w-4 {node.isCurrent ? 'text-indigo-600' : 'text-indigo-500'}" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z"/>
                    </svg>
                    <span class="text-sm">{node.name}</span>
                    {#if node.isCurrent}
                      <span class="ml-auto text-xs text-indigo-600 bg-indigo-200 px-2 py-0.5 rounded-full">Current</span>
                    {/if}
                  </a>
                ...
                {:else if node.type === 'folder'}
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 h-3 w-3 transition-transform {expandedFolders.has(node.path) || node.expanded ? 'rotate-90' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                    </svg>
                    <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 h-3 w-3 text-blue-500" fill="currentColor" viewBox="0 0 24 24">
                ...
                {:else}
                  ...
                  <a
                    ...
                    class="flex w-full items-center rounded-md px-2 py-1 text-left text-sm {currentUnitId() === node.unit?.id ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'text-gray-700 hover:bg-gray-50'}"
                    style="padding-left: {depth * 12 + 20}px"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 h-3 w-3 {currentUnitId() === node.unit?.id ? 'text-blue-600' : 'text-gray-400'}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
