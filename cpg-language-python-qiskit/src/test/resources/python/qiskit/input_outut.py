...
circuit.initialize(str(a), 0); circuit.initialize(str(b), 1); circuit.initialize(str(b), 2) # initialize qubits
...
compiled_circuit = transpile(circuit, simulator); job = simulator.run(compiled_circuit, shots=1000)
...
for bitstring in counts:
    if counts[bitstring] > 300: # some basic threshold to ignore noise
        ...
        e = int(bitstring[-2])
        f = int(bitstring[-3])
        print("In this solution variable: d = %d and e = %d, and f = %d" % (d, e, f))
        if f == 1:
            do_something_very_dangerous()
