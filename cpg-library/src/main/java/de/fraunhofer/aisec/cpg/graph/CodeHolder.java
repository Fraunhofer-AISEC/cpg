...
/**
 * This interface denotes a AST-Node that can contain code. This code is stored as statements. This
 * includes Translation units namespaces and classes as some languages, especially scripting
 * languages allow code placement outside of explicit functions.
 *
 * <p>The reason for not only using a statement property that encapsulate all code in a dummy
 * compound statements is that code can be distributed between functions and an encapsulating
 * compund statement would imply a block of code with a code region containing only the statements.
 */
public interface CodeHolder {
  ...
  /**
   * Adds the specified statement to this code holder. The statements have to be stored as list of
