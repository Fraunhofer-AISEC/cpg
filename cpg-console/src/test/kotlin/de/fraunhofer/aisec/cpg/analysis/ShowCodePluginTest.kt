...
import de.fraunhofer.aisec.cpg.console.ShowCodePlugin
...
class ShowCodePluginTest {
    object TestConfig : ReplConfigurationBase()

    @Test
    fun testExecute() {
        val plugin = ShowCodePlugin().Load(TestConfig)

        // not enough parameters
        var result = plugin.execute(":c")
        assertTrue(result is Command.Result.Failure)

        result = plugin.execute(":c 1")
        assertTrue(result is Command.Result.RunSnippets)
        assertEquals(1, result.snippetsToRun.toList().size)
    }
}
