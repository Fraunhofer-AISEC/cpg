...
class StatementHandler(frontend: PythonLanguageFrontend) :
     ...
     * [ast.statements.expressions.Block] which holds the [ast.statements.CaseStatement] and then all other statements of the
     * [Python.AST.match_case.body].
     *
     * The [ast.statements.CaseStatement] is generated by the [Python.AST.match_case.pattern] and, if available,
     * [Python.AST.match_case.guard]. A `guard` is modeled with an `AND` BinaryOperator in the
     * [ast.statements.CaseStatement.caseExpression]. Its `lhs` is the normal pattern and the `rhs` is the guard.
     ...
     * - An [ast.statements.expressions.AssignExpression] assigning the reference above to the [loopVar] input
     *
    ...
    /**
     * This function "wraps" a list of [Python.AST.BaseStmt] nodes into a [ast.statements.expressions.Block]. Since the list
