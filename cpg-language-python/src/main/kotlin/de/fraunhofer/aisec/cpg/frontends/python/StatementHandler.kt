...
class StatementHandler(frontend: PythonLanguageFrontend) :
     ...
     * [Block] which holds the [Case] and then all other statements of the
     * [Python.AST.match_case.body].
     *
     * The [Case] is generated by the [Python.AST.match_case.pattern] and, if available,
     * [Python.AST.match_case.guard]. A `guard` is modeled with an `AND` BinaryOperator in the
     * [Case.caseExpression]. Its `lhs` is the normal pattern and the `rhs` is the guard.
     ...
     * [LookupScope].
     */
    private fun handleGlobal(global: Python.AST.Global): LookupScope {
     ...
     * into a [LookupScope].
     */
    private fun handleNonLocal(global: Python.AST.Nonlocal): LookupScope {
