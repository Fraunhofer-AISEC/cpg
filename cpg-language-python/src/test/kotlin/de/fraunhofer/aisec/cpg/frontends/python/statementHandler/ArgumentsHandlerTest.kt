...
import de.fraunhofer.aisec.cpg.graph.allParameters
...
class ArgumentsHandlerTest {
    ...
    fun testPosOnlyArguments() {
        val func = result.allFunctions["pos_only_and_args"]
        assertNotNull(func)
    ...
    fun testVarargsArguments() {
        val func = result.allFunctions["test_varargs"]
    ...
    fun testKwOnlyArguments() {
        val func = result.allFunctions["kwd_only_arg"]
    ...
    fun testKwDefaultArguments() {
        val func = result.allFunctions["kw_defaults"]
    ...
    fun testDefaultsArguments() {
        val func = result.allFunctions["defaults"]
    ...
    @Test
    fun testKwArguments() {
        val func = result.allFunctions["kw_args"]
    ...
    fun testMethodDefaults() {
        val func = result.allFunctions["method_with_some_defaults"]

        assertEquals(3, func.allParameters.size)
        val parameterA = func.allParameters["a"]
        val parameterB = func.allParameters["b"]
        val parameterC = func.allParameters["c"]
        assertNotNull(
            parameterA,
    ...
    fun testSignatureMatch() {
        val func = result.allFunctions["kw_args_and_default"]
        assertNotNull(func)
        val funcDefault = func.parameters[1]
        val funcKwargs = func.parameters[2]

        val call = result.allMethods["call"]?.allCalls?.firstOrNull()
        assertNotNull(call)
        assertContains(func.nextDFG, call)

        val call2 = result.allMethods["call2"]?.allCalls?.firstOrNull()
        assertNotNull(call2)
        ...
        val call3 = result.allMethods["call3"]?.allCalls?.firstOrNull()
        assertNotNull(call3)
        ...
        val call4 = result.allMethods["call4"]?.allCalls?.firstOrNull()
        assertNotNull(call4)
