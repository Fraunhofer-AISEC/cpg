...
class OpenStackTest : BaseTest() {
    ...
    fun check(result: TranslationResult) {
        ...
        val problems = checkSensitiveWrites(result)
        problems.forEach {
            val (open, write) = it
            println(
                "${PhysicalLocation.locationLink(open.location)}: Write from potential sensitive source to unprotected file"
            )
            write?.printCode(showNumbers = true, linesAhead = 2)
        }
    }

    fun checkSensitiveWrites(result: TranslationResult): List<Pair<CallExpression, Node?>> {
        return result
            .calls {
                it.name.localName == "open" &&
                    it.arguments[1].evaluate() in listOf("w+", "w", "x", "a")
            ...
            .mapNotNull { call ->
                // the first param is our file
                var file = call.arguments[0].prevFullDFG.singleOrNull()

                // the return value is our io object
                val io = call.nextDFG.singleOrNull() ?: return@mapNotNull null

                // let's follow the DFG from this point to a "write" to
                // our IO
                var paths =
                    io.followNextEOGEdgesUntilHit {
                        it is MemberCallExpression &&
                            it.name.localName == "write" &&
                            io in (it.base?.prevFullDFG ?: listOf()) &&
                            // we are only interested in writes from sensitive
                            // functions
                ...
                return@mapNotNull Pair(call, problemPaths.firstOrNull()?.lastOrNull())
            }
    }
}
