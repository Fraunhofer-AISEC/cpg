 ...
 * [ExpressionHandler], [PatternHandler]). It has no feature-specific `handle...()` functions of its
 * own; those live in the subclasses, one per Rust AST node kind, dispatched via Kotlin `when` on
 * the `ra_ap_syntax` AST enums (`RsItem`, `RsStmt`, `RsExpr`, `RsPat`).
 */
