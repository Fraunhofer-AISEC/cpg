...
class MemoryTest {
    ...
    fun TranslationResult.mapNodesToConcepts(mapToFunctionDeclaration: Boolean) {
        ...
        val keyRefLine1 =
            dRefs.singleOrNull { it.name.localName == "key" && it.location?.region?.startLine == 1 }
        ...
        val getSecretFromServer =
            if (mapToFunctionDeclaration) {
                dFunctions["get_secret_from_server"]
            } else {
                dCalls["get_secret_from_server"]
            }
        val getSecret =
            newGetSecret(
        ...
        val cipher =
            newCipher(
                underlyingNode =
                    assertNotNull(assertNotNull(dCalls["encrypt"]).argumentEdges["cipher"]?.end),
                connect = true,
            )
        ...
        newEncryptOperation(
            underlyingNode = assertNotNull(dCalls["encrypt"]),
            concept = cipher,
            key = key,
            connect = true,
        )
        ...
        descendants<DeleteExpression>().flatMap { delete ->
            delete.operands.map {
