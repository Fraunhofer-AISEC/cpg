...
import de.fraunhofer.aisec.cpg.graph.concepts.file.newFileName
...
/**
 * TODO: update This pass implements the creating of [Concept] and [Operation] nodes for Python file
...
@ExecuteBefore(PythonFileConceptPass::class, true)
class PythonFileConceptPrePass(ctx: TranslationContext) : EOGConceptPass(ctx) {
    ...
    private fun handleGetTempDir(callExpression: CallExpression): Collection<OverlayNode> {
        return listOf(
            newFileName(underlyingNode = callExpression, connect = false).apply {
                this.isTempFile = FileTempFileStatus.TEMP_FILE
            }
        )
    }
}
