...
/**
 * This task implements the creation of [Concept] and [Operation] nodes for Python file
...
class PythonFileConceptTask(ctx: TranslationContext, pass: PythonFileConceptPass) :
    EOGIteratorTask<Node, PythonFileConceptPass, OverlayNode>(ctx, pass) {
    companion object {
        /**
    ...
    internal fun getOrCreateFile(
        ...
        val currentMap = fileCache.computeIfAbsent(pass.currentComponent) { mutableMapOf() }
    ...
    internal fun findFile(
        ...
        return with(pass) {
            expression.getOverlaysByPrevDFG<OpenFile>(stateElement).map { it.file }
        }
    }
