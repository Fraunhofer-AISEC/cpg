...
private fun addPropertiesToClass(
    source: ClassAbstractRepresentation,
    ...
    for (dataProp in source.dataProperties) {
        ...
        var propertyName = dataProp.propertyName.replaceFirstChar { it.lowercase() }

        // need to rename property `id` to prevent naming collisions
        if (propertyName == "id") {
            propertyName =
                source.name.lowercase() + propertyName.replaceFirstChar { it.uppercase() }
            // store name change in property
    ...
    for (objectProp in source.objectProperties) {
...
fun findAllParentsAndSaveToClassAbstraction(
...
private val dataPropertyTypeMap: Map<String, TypeName> =
    mutableMapOf<String, TypeName>().apply {
        put("java.time.Duration", ClassName("kotlin.time", "Duration"))
