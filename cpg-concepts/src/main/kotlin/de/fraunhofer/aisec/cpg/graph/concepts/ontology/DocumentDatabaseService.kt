...
public open class DocumentDatabaseService(
...
) : DatabaseService(anomalyDetections, httpEndpoint, malwareProtection, activityLogging, storage, ips, ports, authenticity, computes, serviceMetadataDocument, transportEncryption, internetAccessibleEndpoint, geoLocation, loggings, redundancies, usageStatistics, creation_time, description, resourceId, labels, name, raw, parent, underlyingNode) {
  init {
    name?.let { this.name = Name(localName = it) }
  }

  override fun equals(other: Any?): Boolean = other is DocumentDatabaseService &&
              super.equals(other)
