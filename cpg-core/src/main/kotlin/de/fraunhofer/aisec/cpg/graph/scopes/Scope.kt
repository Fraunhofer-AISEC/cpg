...
sealed class Scope(
     ...
     *   declared symbols from, in place of the default [Scope.symbols]. This is the hook flow-
     *   sensitive callers (see [de.fraunhofer.aisec.cpg.passes.acceptWithIterateEOG]) use to answer
     *   "which declarations have been reached by this point in the EOG" for
     *   [de.fraunhofer.aisec.cpg.graph.scopes.LocalScope]s, while every other part of this
     *   algorithm (shadowing, [predefinedLookupScopes], wildcard imports, the [HasBuiltins]
        ...
        while (scope != null) {
            list =
                if (localSymbols != null) {
                    localSymbols(scope).filterTo(mutableListOf()) { it.name.localName == symbol }
                } else {
                    scope.symbols[symbol]?.toMutableList()
                }
