 ...
 *
 * Note: For assignments, i.e., using an `=` or `+=`, etc. the [AssignExpr] MUST be used.
 */
open class BinaryOp : Expression(), HasBase, HasOperatorCode, ArgumentHolder, HasType.TypeObserver {
    /** The left-hand expression. */
    @AST
    var lhs: Expression = ProblemExpr("could not parse lhs")
        set(value) {
            disconnectOldLhs()
            field = value
            connectNewLhs(value)
    ...
    @AST
    var rhs: Expression = ProblemExpr("could not parse rhs")
        set(value) {
            disconnectOldRhs()
            field = value
            connectNewRhs(value)
