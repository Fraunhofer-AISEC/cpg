...
class TranslationUnitDecl : Declaration(), DeclarationHolder, StatementHolder, PassTarget {
    ...
    @AST
    val includeEdges: MutableList<PropertyEdge<IncludeDecl>> = ArrayList()

    /** A list of namespaces within this unit. */
    @Relationship(value = "NAMESPACES", direction = Relationship.Direction.OUTGOING)
    @AST
    val namespaceEdges: MutableList<PropertyEdge<NamespaceDecl>> = ArrayList()
    ...
    val includes: List<IncludeDecl>
        get() = unwrap(includeEdges)

    val namespaces: List<NamespaceDecl>
     ...
     * @param clazz the declaration class, such as [FunctionDecl].
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is TranslationUnitDecl) return false
