...
class ClassTemplateDecl : TemplateDecl() {
    ...
    @AST
    val realizationEdges: MutableList<PropertyEdge<RecordDecl>> = ArrayList()

    override val realizations: List<Declaration> by
        PropertyEdgeDelegate(ClassTemplateDecl::realizationEdges)

    fun addRealization(realizedRecord: RecordDecl) {
    ...
    fun removeRealization(realizedRecordDecl: RecordDecl) {
        realizationEdges.removeIf { it.end == realizedRecordDecl }
    }

    override fun addDeclaration(declaration: Declaration) {
        if (declaration is TypeParamDecl || declaration is ParameterDecl) {
            addIfNotContains(super.parameterEdges, declaration)
        } else if (declaration is RecordDecl) {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val that = other as ClassTemplateDecl
