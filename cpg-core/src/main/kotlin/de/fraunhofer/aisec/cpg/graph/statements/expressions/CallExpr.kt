 ...
 * and is connected via the INVOKES edge to its [FunctionDecl].
 */
open class CallExpr : Expression(), HasType.TypeObserver, HasSecondaryTypeEdge, ArgumentHolder {
    /**
     * Connection to its [FunctionDecl]. This will be populated by the [CallResolver]. This will
     * have an effect on the [type]
     */
    @PopulatedByPass(CallResolver::class)
    @Relationship(value = "INVOKES", direction = Relationship.Direction.OUTGOING)
    var invokeEdges = mutableListOf<PropertyEdge<FunctionDecl>>()
        protected set
     ...
     */
    @PopulatedByPass(CallResolver::class)
    var invokes: List<FunctionDecl>
        get(): List<FunctionDecl> {
            val targets: MutableList<FunctionDecl> = ArrayList()
     ...
     * [argumentEdges].
     */
    var arguments by PropertyEdgeDelegate(CallExpr::argumentEdges)
    ...
    override var name: Name
        get() {
            val value = callee
            return if (value is UnaryOp && value.input.type is FunctionPointerType) {
                value.input.name
            } else if (value is BinaryOp && value.rhs.type is FunctionPointerType) {
    ...
    var templateInstantiation: TemplateDecl? = null
        set(value) {
            field = value
            template = value != null
        }
    ...
    override fun typeChanged(newType: Type, src: HasType) {
        ...
        val types =
            invokeEdges
                .map(PropertyEdge<FunctionDecl>::end)
                .mapNotNull {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is CallExpr) return false
