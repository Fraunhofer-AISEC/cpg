...
/** A method declaration is a [FunctionDecl] that is part of to a specific [RecordDecl] . */
open class MethodDecl : FunctionDecl() {
    var isStatic = false

    /** The [RecordDecl] this method is part of. This can be empty if we do not know about it. */
    open var recordDecl: RecordDecl? = null
     ...
     * created (usually in the form of a [VariableDecl] and added to the scope of the [MethodDecl]
     * by a language frontend. Furthermore, it must be manually set to the [receiver] property of
     * the method, since the scope manager cannot do this. If the name of the receiver, e.g.,
     * `this`, is used anywhere in the method body, a [Reference] must be created by the language
     * frontend, and its [Reference.refersTo] property must point to this [receiver]. The latter is
     ...
     * needs to be taken that for the first argument of the method, a [VariableDecl] is created and
     * stored in [receiver]. All other arguments must then be processed normally (usually into a
     * [ParameterDecl]). This is also important because from the "outside" the method only has the
     ...
     */
    @AST var receiver: VariableDecl? = null
}
