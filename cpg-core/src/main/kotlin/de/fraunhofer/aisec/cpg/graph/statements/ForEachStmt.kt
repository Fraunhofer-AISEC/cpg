...
class ForEachStmt : Statement(), BranchingNode, StatementHolder {
    /**
    ...
    override fun addStatement(s: Statement) {
        if (variable == null) {
            ...
            statement = s
        } else if (statement !is CompoundStmt) {
            val newStmt = newCompoundStmt()
            statement?.let { newStmt.addStatement(it) }
            newStmt.addStatement(s)
            statement = newStmt
        } else {
            (statement as? CompoundStmt)?.addStatement(s)
        }
    }

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is ForEachStmt) return false
