...
open class FunctionDecl : ValueDecl(), DeclarationHolder {
    /** The function body. Usually a [CompoundStmt]. */
    ...
    var recordEdges = mutableListOf<PropertyEdge<RecordDecl>>()

    /** The list of function parameters. */
    @Relationship(value = "PARAMETERS", direction = Relationship.Direction.OUTGOING)
    @AST
    var parameterEdges = mutableListOf<PropertyEdge<ParameterDecl>>()

    /** Virtual property for accessing [parameterEdges] without property edges. */
    var parameters by PropertyEdgeDelegate(FunctionDecl::parameterEdges)

    /** Virtual property for accessing [parameterEdges] without property edges. */
    var records by PropertyEdgeDelegate(FunctionDecl::recordEdges)
    ...
    val overriddenBy = mutableListOf<FunctionDecl>()

    @Relationship(value = "OVERRIDES", direction = Relationship.Direction.OUTGOING)
    val overrides = mutableListOf<FunctionDecl>()
    ...
    var definition: FunctionDecl? = null
        get() {
    ...
    fun hasSameSignature(targetFunctionDecl: FunctionDecl): Boolean {
    ...
    fun hasSignature(targetSignature: List<Type>): Boolean {
        val signature =
            parameters
                .stream()
                .sorted(Comparator.comparingInt(ParameterDecl::argumentIndex))
    ...
    fun isOverrideCandidate(other: FunctionDecl): Boolean {
        return other.name.localName == name.localName &&
            other.type == type &&
            other.signature == signature
    }

    fun addOverriddenBy(c: Collection<FunctionDecl>) {
        for (functionDeclaration in c) {
            addOverriddenBy(functionDeclaration)
        }
    }

    fun addOverriddenBy(functionDecl: FunctionDecl) {
        addIfNotContains(overriddenBy, functionDecl)
    }

    fun addOverrides(functionDecl: FunctionDecl) {
    ...
    fun <T> getBodyStatementAs(i: Int, clazz: Class<T>): T? {
        if (body is CompoundStmt) {
            val statement = (body as CompoundStmt).statements[i]
    ...
    /**
     * A list of default expressions for each item in [parameters]. If a [ParameterDecl] has no
     * default, the list will be null at this index. This list must have the same size as
     * [parameters].
     */
    ...
    fun addParameter(parameterDecl: ParameterDecl) {
    ...
    fun removeParameter(parameterDecl: ParameterDecl) {
