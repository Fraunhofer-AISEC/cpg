 ...
 * the scope and [resetToGlobal] when they first handle a new [ast.declarations.TranslationUnitDeclaration].
 * Afterward the currently valid "stack" of scopes within the tree can be accessed.
 *
 * If a language frontend encounters a [ast.declarations.Declaration] node, it MUST call [addDeclaration], rather
...
class ScopeManager(override var ctx: TranslationContext) : ScopeProvider, ContextProvider {
     ...
     *
     * Afterward, all calls to [addDeclaration] will be distributed to the [ast.DeclarationHolder] that
     ...
     * i.e. translation units, represented by different [ast.declarations.NamespaceDeclaration] nodes. But, in order
     ...
     * - we update the scope map so that the current [ast.declarations.NamespaceDeclaration] points to the existing
     *   [NamespaceScope]
    ...
    /**
     * This function MUST be called when a language frontend first handles a [ast.declarations.Declaration]. It adds
    ...
    /**
     * This function retrieves the [ast.statements.LabelStatement] associated with the [labelString]. This depicts
     * the feature of some languages to attach a label to a point in the source code and use it as
     * the target for control flow manipulation, e.g. [ast.statements.BreakStatement], [ast.statements.GotoStatement].
     */
     ...
     * [ast.declarations.ImportDeclaration] and therefore can not directly be found in the [nameScopeMap].
     *
    ...
    /**
     * Returns the [ast.declarations.TranslationUnitDeclaration] that should be used for inference, especially for
     * global declarations.
     *
...
/**
 * [SignatureResult] will be the result of the function [ast.declarations.FunctionDeclaration.matchesSignature] which
 * calculates whether the provided [ast.statements.expressions.CallExpression] will match the signature of the current
...
data class CallResolutionResult(
    /** The original expression that triggered the resolution. Most likely a [ast.statements.expressions.CallExpression]. */
    ...
    /**
     * A set of candidate symbols we discovered based on the [ast.statements.expressions.CallExpression.callee] (using
     * [ScopeManager.lookupSymbolByName]), more specifically a list of [ast.declarations.FunctionDeclaration] nodes.
     */
     ...
     * [ast.declarations.FunctionDeclaration.matchesSignature] for each function in [viableFunctions].
     */
    ...
    enum class SuccessKind {
         ...
         * in the [ast.statements.expressions.MemberExpression.base]. In this case, all best viable functions of each of the
