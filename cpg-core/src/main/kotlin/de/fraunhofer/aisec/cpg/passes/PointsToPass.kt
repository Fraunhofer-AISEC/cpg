...
open class PointsToPass(ctx: TranslationContext) : EOGStarterPass(ctx, orderDependencies = true) {
                    ...
                    parentFD.functionSummary
                        ...
                        .addAll(
                            doubleState.getValues(retval).map {
                                FunctionDeclaration.FSEntry(0, it.first, 1, "")
                            }
                        )
                }
            }
                    ...
                    doubleState =
                        lattice.push(
                            ...
                            GeneralStateEntryElement(
                                ...
                                PowersetLattice.Element(
                                    Pair(arg, equalLinkedHasSetOf(callingContext))
                                ),
                            ),
                        )
                                        ...
                                        lattice.push(
                                            ...
                                            GeneralStateEntryElement(
                                                ...
                                                PowersetLattice.Element(
                                                    Pair(
                                                        derefValue,
                                                        equalLinkedHasSetOf(callingContext),
                                                    )
                                                ),
                                            ),
                                        )
    ...
    private fun handleCallExpression(
        ...
        var callingContextOut = CallingContextOut(setOf(currentNode))
            ...
            val lastWrites: IdentitySet<Pair<Node, EqualLinkedHashSet<Any>>> =
                ...
                values.mapTo(IdentitySet()) {
                    if (it.third.singleOrNull() == true) Pair(it.second, it.third)
                        ...
                        Pair(
                            it.second,
                            equalLinkedHasSetOf(*(it.third + callingContextOut).toTypedArray()),
                        )
                }
    ...
    private fun handleAssignExpression(
        ...
        if (currentNode.lhs.size == 1 && currentNode.rhs.size == 1) {
            ...
            val lastWrites =
                destinations.mapTo(IdentitySet()) { Pair(it, equalLinkedHasSetOf<Any>(false)) }
            doubleState =
                doubleState.updateValues(
                    lattice,
                    sources,
            ...
            doubleState =
                lattice.pushToDeclarationsState(
                    ...
                    DeclarationStateEntryElement(
                        ...
                        PowersetLattice.Element(Pair(currentNode, equalLinkedHasSetOf<Any>(false))),
                    ),
                )
        }
        return doubleState
                ...
                for (i in 0..paramDepth) {
                    ...
                    val declStateElement =
                        DeclarationStateEntryElement(
                            ...
                            PowersetLattice.Element(Pair(pmv, equalLinkedHasSetOf())),
                        )
                    val genStateElement =
                        GeneralStateEntryElement(
                            PowersetLattice.Element(addresses),
...
fun PointsToStateElement.fetchElementFromDeclarationState(
...
): IdentitySet<Triple<Node, Boolean, String>> {
    val ret = identitySetOf<Triple<Node, Boolean, String>>()
    ...
    if (isGlobal(addr)) {
        ...
        if (elements.isNullOrEmpty()) {
            ...
            ret.add(Triple(newEntry, false, ""))
        } else elements.map { ret.add(Triple(it.first, it.second, "")) }
    ...
    return when (node) {
        is PointerReference -> {
            ...
            return (node.input as Reference).refersTo?.memoryAddresses?.mapTo(IdentitySet()) {
                Pair<Node, EqualLinkedHashSet<Any>>(it, equalLinkedHasSetOf())
            } ?: identitySetOf(Pair(node, equalLinkedHasSetOf()))
        }
            ...
            this.getAddresses(node).forEach { addr ->
                ...
                if (lastWrite?.isNotEmpty() == true)
                    lastWrite.mapTo(IdentitySet()) {
                        ret.add(Pair(it.first, equalLinkedHasSetOf(it.second)))
                    }
            ...
            this.getAddresses(node)
                ...
                .flatMapTo(IdentitySet()) {
                    this.declarationsState[it]?.third!!.map {
                        Pair(it.first, equalLinkedHasSetOf<Any>(it.second))
                    }
                }
    }
}
    ...
    return when (node) {
        ...
        is PointerDereference -> {
            ...
            inputVal.forEach { input ->
                retVal.addAll(
                    fetchElementFromDeclarationState(input, true).map { Pair(it.first, it.second) }
                )
            }
            retVal
        }
        ...
        is Reference -> {
            ...
            this.getAddresses(node).forEach { addr ->
                ...
                if (isGlobal(node))
                    retVals.addAll(
                        fetchElementFromDeclarationState(addr).map { Pair(it.first, false) }
                    )
