...
class IncludeDecl : Declaration() {
    ...
    @AST
    private val includeEdges: MutableList<PropertyEdge<IncludeDecl>> = ArrayList()

    @Relationship(value = "PROBLEMS", direction = Relationship.Direction.OUTGOING)
    @AST
    private val problemEdges: MutableList<PropertyEdge<ProblemDecl>> = ArrayList()

    var filename: String? = null

    val includes: List<IncludeDecl> by PropertyEdgeDelegate(IncludeDecl::includeEdges)

    val problems: List<ProblemDecl> by PropertyEdgeDelegate(IncludeDecl::problemEdges)

    fun addInclude(includeDecl: IncludeDecl?) {
        if (includeDecl == null) return
        val propertyEdge = PropertyEdge(this, includeDecl)
        propertyEdge.addProperty(Properties.INDEX, includeEdges.size)
        includeEdges.add(propertyEdge)
    }

    fun addProblems(c: Collection<ProblemDecl>) {
        for (problemDeclaration in c) {
            addProblem(problemDeclaration)
        }
    }

    fun addProblem(problemDecl: ProblemDecl) {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is IncludeDecl) return false
