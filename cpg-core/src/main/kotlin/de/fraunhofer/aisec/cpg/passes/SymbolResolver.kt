 ...
 *
 * A field access is modeled with a [de.fraunhofer.aisec.cpg.graph.expressions.MemberAccess]. After AST building, its base and member
 * references are set to [de.fraunhofer.aisec.cpg.graph.expressions.Reference] stubs. This pass resolves those references and makes the member
 ...
 * same unknown declaration. [de.fraunhofer.aisec.cpg.graph.expressions.Reference] stubs are removed from the graph after being resolved.
 *
 * Accessing a local variable is modeled directly with a [de.fraunhofer.aisec.cpg.graph.expressions.Reference]. This step of the pass doesn't
 * remove the [de.fraunhofer.aisec.cpg.graph.expressions.Reference] nodes like in the field usage case but rather makes their "refersTo" point
 * to the appropriate [ValueDeclaration].
 *
 * Resolves [de.fraunhofer.aisec.cpg.graph.expressions.Call] and [de.fraunhofer.aisec.cpg.graph.expressions.New] targets.
 *
 * A [de.fraunhofer.aisec.cpg.graph.expressions.Call] specifies the method that wants to be called via [de.fraunhofer.aisec.cpg.graph.expressions.Call.name]. The call target is a
 ...
 * accordingly.
 *
 * Constructor calls with [de.fraunhofer.aisec.cpg.graph.expressions.Construction] are resolved in such a way that their
 * [de.fraunhofer.aisec.cpg.graph.expressions.Construction.instantiates] points to the correct [Record]. Additionally, the
 * [de.fraunhofer.aisec.cpg.graph.expressions.Construction.constructor] is set to the according [Constructor].
 *
...
open class SymbolResolver(ctx: TranslationContext) : EOGStarterPass(ctx) {
    ...
    /**
     * This function handles symbol resolving for a [de.fraunhofer.aisec.cpg.graph.expressions.Reference]. After a successful lookup of the
     * symbol contained in [de.fraunhofer.aisec.cpg.graph.expressions.Reference.name], the property [de.fraunhofer.aisec.cpg.graph.expressions.Reference.refersTo] is set to the best
     * (or only) candidate.
     *
     ...
     *   [de.fraunhofer.aisec.cpg.graph.expressions.Reference.name]. This can either result in an "unqualified" or "qualified" lookup,
     *   depending on the name.
     * - The results of the lookup are stored in [de.fraunhofer.aisec.cpg.graph.expressions.Reference.candidates]. The purpose of this is
     ...
     *   option out of the candidates (if the reference is part of the [de.fraunhofer.aisec.cpg.graph.expressions.Call.callee]).
     * - In the next step, we need to decide whether we are resolving a standalone reference (which
     *   most likely points to a [Variable]) or if we are part of a [de.fraunhofer.aisec.cpg.graph.expressions.Call.callee]. In the first
     *   case, we can directly assign [de.fraunhofer.aisec.cpg.graph.expressions.Reference.refersTo] based on the candidates (at the moment we
     *   only assign it if we have exactly one candidate). In the second case, we are finished and
     *   let [handleCall] take care of the rest once the EOG reaches the appropriate [de.fraunhofer.aisec.cpg.graph.expressions.Call] (which
    ...
    /**
     * This function handles resolving of a [de.fraunhofer.aisec.cpg.graph.expressions.MemberAccess] in the [ScopeManager.currentRecord]. This
     ...
     * current class and its parent classes. Then, if we resolve a [de.fraunhofer.aisec.cpg.graph.expressions.MemberCall], we abort (and later
    ...
    companion object {
         ...
         *   [Construction] uses AST as well as the [Variable]/[de.fraunhofer.aisec.cpg.graph.expressions.New]
         */
