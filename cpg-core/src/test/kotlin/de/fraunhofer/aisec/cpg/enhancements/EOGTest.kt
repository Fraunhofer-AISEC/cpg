    ...
    fun testIf(relPath: String, refNodeString: String) {
        ...
        assertTrue(
            Util.eogConnect(
                ...
                n = ifSimple,
                props = mutableMapOf(Properties.BRANCH to true),
                refs = listOfNotNull(ifSimple.thenStatement)
            )
        )
        assertTrue(
            Util.eogConnect(
        ...
        assertTrue(
            Util.eogConnect(
                ...
                n = ifBranched,
                props = mutableMapOf(Properties.BRANCH to true),
                refs = listOfNotNull(ifBranched.thenStatement)
            )
        )
        assertTrue(
            Util.eogConnect(
                ...
                refs = listOfNotNull(ifBranched.elseStatement)
            )
        )
        ...
        assertTrue(
            Util.eogConnect(
                ...
                en = Util.Edge.ENTRIES,
                n = ifBranched,
                refs = listOfNotNull(ifBranched.condition)
            )
        )
    }
    ...
    fun testLoops(relPath: String, refNodeString: String?) {
        ...
        assertTrue(
            Util.eogConnect(
                ...
                en = Util.Edge.ENTRIES,
                n = dostat,
                refs = listOfNotNull(dostat.condition)
            )
        )
        // Assert: All EOGs going into the loop branch come from the condition
        assertTrue(
            Util.eogConnect(
                en = Util.Edge.EXITS,
                n = prints[1],
                refs = listOfNotNull(dostat.statement)
            )
        )
        assertTrue(
            Util.eogConnect(
    ...
    fun testSwitch(relPath: String, refNodeString: String) {
        ...
        assertTrue(
            Util.eogConnect(
                ...
                n = prints[0],
                cr = Connect.SUBTREE,
                refs = listOfNotNull(swch.selector)
            )
        )
        assertTrue(
            Util.eogConnect(
