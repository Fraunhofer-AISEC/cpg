...
class AssignExprTest {
    ...
    fun propagateSimple() {
        with(TestLanguageFrontend()) {
            ...
            val stmt = newAssignExpr(lhs = listOf(refA), rhs = listOf(refB))
                ...
                translationResult {
                    translationUnit {
                        ...
                        function("main") {
                            ...
                            val call = newCallExpr(refFunc)

                            // Assignment from "func()" to "a" and "err".
                            val stmt = newAssignExpr(lhs = listOf(refA, refErr), rhs = listOf(call))

                            body = newCompoundStmt()
                            body as CompoundStmt += stmt
                        }
                    }
                }
            }
