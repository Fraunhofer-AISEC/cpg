...
class ExpressionHandler(lang: JavaLanguageFrontend) :
    ...
    private fun handleLambdaExpr(expr: Expression): Statement {
        ...
        val lambda =
            newLambdaExpression(rawNode = lambdaExpr).withChildren {
                val anonymousFunction =
                    newFunctionDeclaration("", rawNode = lambdaExpr).withChildren(true) {
                        ...
                        val functionType = FunctionType.computeType(it)
                        it.type = functionType
                        it.body = frontend.statementHandler.handle(lambdaExpr.body)
                    }
    ...
    private fun handleCastExpr(expr: Expression): Statement {
        ...
        val castExpression =
            newCastExpression(rawNode = expr).withChildren {
                val expression =
    ...
    private fun handleArrayCreationExpr(expr: Expression): Statement {
        ...
        val creationExpression =
            newNewArrayExpression(rawNode = expr).withChildren { creationExpression ->
                // in Java, an array creation expression either specifies an initializer or
                // dimensions
    ...
    private fun handleArrayInitializerExpr(expr: Expression): Statement {
        ...
        val initList =
            newInitializerListExpression(arrayType, rawNode = expr).withChildren {
                val initializers =
    ...
    private fun handleArrayAccessExpr(expr: Expression): SubscriptExpression {
        ...
        val arraySubsExpression =
            newSubscriptExpression(rawNode = expr).withChildren { arraySubsExpression ->
    ...
    private fun handleConditionalExpression(expr: Expression): ConditionalExpression {
        ...
        val conditionExpression =
            newConditionalExpression(superType, expr).withChildren {
                it.condition =
    ...
    private fun handleAssignmentExpression(expr: Expression): AssignExpression {
        ...
        val assign =
            newAssignExpression(assignExpr.operator.asString(), rawNode = assignExpr).withChildren {
    ...
    private fun handleVariableDeclarationExpr(expr: Expression): DeclarationStatement {
        ...
        val declarationStatement =
            newDeclarationStatement(rawNode = expr).withChildren {
                for (variable in variableDeclarationExpr.variables) {
                    ...
                    it.addToPropertyEdgeDeclaration(declaration)
    ...
    private fun handleFieldAccessExpression(
        ...
        memberExpression =
            newMemberExpression(
                ...
                .withChildren { memberExpression ->
                    var base: de.fraunhofer.aisec.cpg.graph.statements.expressions.Expression
                    // first, resolve the scope. this adds the necessary nodes, such as IDENTIFIER
                    // for the
                    // scope.
                    ...
                    if (scope.isNameExpr) {
                        ...
                        try {
                            ...
                            baseType =
                                this.objectType(resolve.asField().declaringType().qualifiedName)
                        } catch (ex: RuntimeException) {
                            isStaticAccess = true
                            val typeString = frontend.recoverTypeFromUnsolvedException(ex)
                            if (typeString != null) {
                                ...
                                val qualifiedNameFromImports =
                                    frontend.getQualifiedNameFromImports(name)
                                baseType =
                        ...
                        base =
                            newReference(scope.asNameExpr().nameAsString, baseType, rawNode = scope)
                        ...
                        while (tester is MemberExpression) {
                            // we need to check if any base is only a static access, otherwise, this
                            // is a
                            // member
                            // access
                            // to this base
    ...
    private fun handleBinaryExpression(expr: Expression): BinaryOperator {
        ...
        val binaryOperator =
            newBinaryOperator(binaryExpr.operator.asString(), rawNode = binaryExpr).withChildren {
                // first, handle the target. this is the first argument of the operator call
                it.lhs =
    ...
    private fun handleMethodCallExpression(expr: Expression): CallExpression {
        ...
        callExpression =
            newMemberCallExpression(isStatic = isStatic, rawNode = expr).withChildren {
                ...
                if (o.isPresent) {
                    ...
                    // b) It could refer to some method that was statically imported using an
                    // asterisks
                    // import
                    //
                    // Luckily, the resolved method already hints whether this is a static call
                    if (isStatic) {
                        // In case this is a static call, we need some additional information from
                        // the
                        // resolved
                        // method
                        val baseType: Type
                        ...
                        // Since it is possible to omit the "this" keyword, some methods in java do
                        // not
                        // have
                        // a base.
                        // However, they are still scoped to the local class, meaning we should
                        // insert
                        // an
                        // implicit
                ...
                for (i in arguments.indices) {
                    ...
                    it.addArgument(argument ?: newProblemExpression("Could not parse the argument"))
                }
            }
    ...
    private fun handleObjectCreationExpr(expr: Expression): NewExpression {
        ...
        val newExpression =
            newNewExpression(t, rawNode = expr).withChildren { newExpression ->
                newExpression.initializer =
                    newConstructExpression(rawNode = expr).withChildren { ctor ->
                        ctor.type = t
                        ...
                        if (objectCreationExpr.anonymousClassBody.isPresent) {
                            // We have an anonymous class and will create a RecordDeclaration for it
                            // and add
                            // all the
                            // implemented methods.
                            val locationHash = frontend.locationOf(objectCreationExpr)?.hashCode()

                            // We use the hash of the location to distinguish multiple instances of
                            // the
                            // anonymous
                            // class' superclass
                            val anonymousClassName = "$constructorName$locationHash"
                            val anonymousRecord =
                                newRecordDeclaration(anonymousClassName, "class").withChildren(
                                    true
                                ) { anonymousRecord ->
                                    anonymousRecord.addSuperClass(objectType(constructorName))
                                    val anonymousClassBody =
                                        objectCreationExpr.anonymousClassBody.get()
                                    for (classBody in anonymousClassBody) {
                                        // Whatever is implemented in the anonymous class has to be
                                        // added to the
                                        // record
                                        // declaration
                                        val classBodyDecl =
                                            frontend.declarationHandler.handle(classBody)
